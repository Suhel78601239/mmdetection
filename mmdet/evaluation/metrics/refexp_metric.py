...
class RefExpMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for result in results:
            ...
            ann_ids = self.coco.getAnnIds(imgIds=img_id)
            if len(ann_ids) == 0:
                continue  # skip if no GT annotations

            img_info = self.coco.loadImgs(img_id)[0]

            # Load all GT bboxes for this image
            ...
            # IoU between predictions and all GT bboxes
            ious = bbox_overlaps(result['bboxes'], gt_bboxes)

            for k in self.topk:
                # check top-k predictions: is any IoU >= threshold?
                topk_ious = ious[:k] if len(ious) >= k else ious
                if topk_ious.max() >= self.iou_thrs:
                    dataset2score[img_info['dataset_name']][k] += 1.0
        ...
        for key, value in dataset2score.items():
            for k in self.topk:
                try:
                    ...
                    print(f"Warning: {e} for dataset {key}")
        ...
        # mean precision across datasets (used for best_ckpt saving)
        out_results = {'mean_precision': mean_precision / 9.0}
